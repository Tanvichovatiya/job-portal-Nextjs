...
function registerJobHandlers(io, socket) {
    // CREATE JOB
    socket.on("createJob", async (data, cb) => {
        try {
            const userId = socket.data.userId;
            if (!userId)
                return cb({ status: "error", message: "Not authenticated" });
    ...
    socket.on("getJobs", async (payload, cb) => {
        try {
    ...
    // get job By id
    // GET SINGLE JOB
    socket.on("getJobById", async (jobId, cb) => {
        try {
            if (!jobId)
                return cb({ status: "error", message: "Job ID required" });
