...
exports.registerNetworkHandlers = registerNetworkHandlers;
...
function registerNetworkHandlers(io, socket) {
    // ✅ Get All Users
    ...
    socket.on("respondConnectionRequest", async ({ connectionId, action }, cb) => {
        try {
            ...
            const conn = await prisma_1.prisma.connection.findUnique({ where: { id: connectionId } });
            if (!conn)
                return cb?.({ status: "error", message: "Connection not found" });
            if (conn.receiverId !== userId)
                return cb?.({ status: "error", message: "Not authorized" });
            const receiver = await prisma_1.prisma.user.findUnique({ where: { id: userId } });
            const requesterId = conn.requesterId; // A
            // ✅ Step 1: Update or delete connection based on action
            if (action === "accepted") {
                await prisma_1.prisma.connection.update({
                    where: { id: connectionId },
                    data: { status: "accepted" },
                });
            }
            else if (action === "rejected") {
                await prisma_1.prisma.connection.delete({ where: { id: connectionId } });
            }
